    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.put(JSONBIN_URL, json=data, headers=HEADERS) as resp:
                if resp.status != 200:
                    print(f"Ошибка сохранения в JSONBin: {resp.status}")
    except Exception as e:
...
async def cmd_start(message: Message) -> None:
    await message.answer("🇨🇳 <b>Добро пожаловать!</b>\n\nЭто бот для проверки знаний китайского языка.\n⚠️ Каждый тест можно пройти <b>только один раз</b>.", reply_markup=build_main_keyboard(), parse_mode="HTML")
