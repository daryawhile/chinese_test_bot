...
router = Router()
COMPLETED_FILE = Path(__file__).parent / "completed.json"
        ...
        with open(COMPLETED_FILE, "r", encoding="utf-8") as f:
            ...
            for user_id, user_data in data.items():
                if isinstance(user_data, list):
                    new_data = {}
                    for lesson_id in user_data:
                        if lesson_id in TESTS:
                            new_data[lesson_id] = {"score": "?", "total": len(TESTS[lesson_id]["questions"]),
                                                   "date": "ранее"}
                    data[user_id] = new_data
            if any(isinstance(v, list) for v in data.values()):  # простая проверка, была ли миграция
...
def get_user_results(user_id: int) -> dict:
    return load_completed().get(str(user_id), {})


def mark_completed(user_id: int, lesson_id: str, score: int, total: int) -> None:
    data = load_completed()
    key = str(user_id)
    if key not in data:
        data[key] = {}
...
def build_answer_keyboard(lesson_id: str, q_index: int, options: list[str]) -> InlineKeyboardMarkup:
    buttons = [[InlineKeyboardButton(text=opt, callback_data=f"ans_{lesson_id}_{q_index}_{i}")] for i, opt in
               enumerate(options)]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
    ...
    if total_questions > 0:
        lines.extend(
            ["───────────────", f"📈 <b>Общая статистика:</b>", f"   Пройдено тестов: {completed_count}/{len(TESTS)}",
             f"   Правильных ответов: {total_score}/{total_questions} ({round(total_score / total_questions * 100)}%)"])
    else:
        lines.extend(
            ["───────────────", f"📈 <b>Общая статистика:</b>", f"   Пройдено тестов: {completed_count}/{len(TESTS)}"])
    return "\n".join(lines)
...
async def cmd_start(message: Message) -> None:
    await message.answer(
        ...
        reply_markup=build_main_keyboard(), parse_mode="HTML")


@router.callback_query(F.data == "show_results")
async def show_results(callback: CallbackQuery) -> None:
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад к урокам", callback_data="back_to_lessons")]])
    await callback.message.edit_text(format_results(callback.from_user.id), reply_markup=keyboard, parse_mode="HTML")
...
async def start_lesson(callback: CallbackQuery) -> None:
    ...
    if lesson_id not in TESTS or is_completed(user_id, lesson_id):
    ...
    await callback.message.edit_text(format_question(lesson_id, 0),
                                     reply_markup=build_answer_keyboard(lesson_id, 0, q["options"]), parse_mode="HTML")
    await callback.answer()
...
async def handle_answer(callback: CallbackQuery) -> None:
    ...
    if next_q < total:
        ...
        await callback.message.edit_text(f"{feedback}\n\n{format_question(lesson_id, next_q)}",
                                         reply_markup=build_answer_keyboard(lesson_id, next_q, next_q_data["options"]),
                                         parse_mode="HTML")
    else:
        score = session["score"]
        mark_completed(user_id, lesson_id, score, total)
